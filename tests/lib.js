...
test('filenameHasLoaders', (t) => {
  ...
  t.false(filenameHasLoaders('/filename'), 'Should handle no loader');
  t.false(filenameHasLoaders('http://doman/path'), 'Should handle URL');
});
...
test('isFilenameAbsolute', (t) => {
  ...
  t.false(isFilenameAbsolute('./filename'), './filename is not absolute');
  t.true(isFilenameAbsolute('/filename'), '/filename is absolute');
  t.true(
...
test('filenameHasSchema', (t) => {
  t.plan(3);
  t.false(filenameHasSchema('/filename'), '/filename has no schema');
  t.true(filenameHasSchema('http://filename'), 'http://filename has a schema');
  t.true(
...
test('makeUrl', (t) => {
  t.plan(7);
  t.equal(makeUrl('/filename'), 'file:///filename', 'Should handle local file');
  t.equal(makeUrl('http://filename'), 'http://filename', 'Should handle URL');
  t.equal(
...
test('makeLinkText', (t) => {
  t.plan(4);
  t.equal(makeLinkText('/filename'), '/filename', 'Should handle filename');
  t.equal(
